...
timestep_group = np.array([1,1,1,1,1,1,1,0.5,0.2,0.2]) # corresponding to energies
print('timestep group')
print(timestep_group)

deltastep_group = np.array([2000, 2000, 2000, 2000, 2000, 2000, 2000, 3000, 4000, 4000]) # corresponding to energies
print('deltastep group')
print(deltastep_group)
...
for i in range(len(angle_group)):
    for j in range(len(energy_group)):
        ...
        copyfile("Ar.on.polystyrene.lammps", path+"/Ar.on.polystyrene.lammps")
        with in_place.InPlace(path+"/Ar.on.polystyrene.lammps") as file:
            for line in file:
                ...
                if line.startswith("variable dt equal YYY1"):
                    line = line.replace('YYY1', str(timestep_group[j]))
                if line.startswith("variable delta_step_insert equal YYY2"):
                    line = line.replace('YYY2', str(deltastep_group[j]))
                file.write(line)
