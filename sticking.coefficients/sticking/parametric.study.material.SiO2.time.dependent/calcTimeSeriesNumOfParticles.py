...
pipeline.modifiers.append(ExpressionSelectionModifier(expression = 'ParticleType==3 && Position.Z<=42'))
pipeline.modifiers.append(TimeSeriesModifier(operate_on = 'ExpressionSelection.count', sampling_frequency=10, time_attribute='Timestep'))
data = pipeline.compute()
time_series_1 = data.tables['time-series'].xy()

pipeline2 = import_file(sys.argv[1])
pipeline2.modifiers.append(ExpressionSelectionModifier(expression = 'ParticleType==4 && Position.Z<=42'))
pipeline2.modifiers.append(TimeSeriesModifier(operate_on = 'ExpressionSelection.count', sampling_frequency=10, time_attribute='Timestep'))
data = pipeline2.compute()
time_series_2 = data.tables['time-series'].xy()

print(time_series_1[ : , 0])
fig = plt.figure()
...
fig = plt.figure()
ratio = numpy.divide(time_series_2[ : , 1], time_series_1[ : , 1], out=numpy.zeros_like(time_series_1[ : , 1]), where=time_series_1[ : , 1]!=0)
plt.plot(time_series_2[ : , 0], ratio)
plt.xlabel('Timestep (4E6 steps = 1 ns)')
plt.ylabel('F/C ratio')
plt.savefig('ratio.attached.F.over.C.vs.timestep.pdf')
