...
from ovito.modifiers import ExpressionSelectionModifier, TimeSeriesModifier, CreateBondsModifier, DeleteSelectedModifier
...
DICT_BONDS = {('Si','Si') : {'bondID' : 1, 'cutoff' : 2.5},
              ('Si','C')  : {'bondID' : 2, 'cutoff' : 2.2},
              ('Si','F')  : {'bondID' : 3, 'cutoff' : 1.83},
              ('C','F')   : {'bondID' : 4, 'cutoff' : 1.5},
              ('C','O')   : {'bondID' : 5, 'cutoff' : 1.6},
              ('Si','O')  : {'bondID' : 6, 'cutoff' : 1.9}, }


def countNumOfSelectedParticles(fileName='Si.300K.sticked.10.dat', SELECTED_PARTICLE_TYPE = 3, SELECTED_BOND_TYPES = [2, 5],
                                delete_below_z = 0, delete_above_z = 0, particle_ID_incident_minimum = 0):
    pipeline = import_file(fileName)

    # delete atoms below the specified z value
    if delete_below_z > 0:
        pipeline.modifiers.append(ExpressionSelectionModifier(expression = 'Position.Z<='+str(delete_below_z)))
        pipeline.modifiers.append(DeleteSelectedModifier())
    # delete atoms above the specified z value
    if delete_above_z > 0:
        pipeline.modifiers.append(ExpressionSelectionModifier(expression = 'Position.Z>='+str(delete_above_z)))
        pipeline.modifiers.append(DeleteSelectedModifier())
    ...
    df_selected_particles_by_bond_type = df_particles.iloc[pd.concat([df_bonds[df_bonds['Bond Type'].isin(SELECTED_BOND_TYPES)]['Topology.1'], df_bonds[df_bonds['Bond Type'].isin(SELECTED_BOND_TYPES)]['Topology.2']]).unique()]
    ...
    if particle_ID_incident_minimum > 0:
        df_selected_particles = df_selected_particles[df_selected_particles['Particle ID'] >= particle_ID_incident_minimum]

    print(df_selected_particles)
    return df_selected_particles.shape[0]


files = [x for x in listdir() if x.endswith('.dat') and x.startswith('SiO2.300K.sticked.')]
sum = 0
max = 0
for file in files:
    print(file)
    # sum = sum + countNumOfSelectedParticles(fileName=file, delete_below_z=25, delete_above_z=38, particle_ID_incident_minimum=3001)
    sum = sum + countNumOfSelectedParticles(fileName=file, SELECTED_PARTICLE_TYPE = 1, SELECTED_BOND_TYPES = [1, 6],
                                            delete_below_z=25, delete_above_z=38, particle_ID_incident_minimum=3001)
    max = max + 5
    print('sum =', sum)
    print('max =', max)
    print()
